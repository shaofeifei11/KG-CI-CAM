...
from dataset.CUB_dataset_randomresizecrop import CUB_Dataset_RandomResizeCrop
...
from dataset.ImageNet_dataset_randomresizecrop import ImageNet_Dataset_Randomresizecrop
from model.imagenet_inceptionV3_nonlocal_distri import Imagenet_InceptionV3_NonLocal_Distri
...
from test_distri import inference
...
#     k[where_are_inf] = torch.tensor(0)
#     entropy = -1 * torch.sum(torch.multiply(x, k), dim=-1)
...
if __name__ == '__main__':
    ...
    if args.dataset == "cub":
        ...
        if args.backbone == "vgg16" and args.new_image_backbone == 0:
            ...
            net = Imagenet_InceptionV3_NonLocal_Distri(args)
    ...
    #             labels = labels.long()
    #             optimizer.zero_grad()
    #             cam_up, out_up, pred_sort_up, pred_ids_up, cam_down, out_down, pred_sort_down, pred_ids_down, diff_out, diff_ids, aux_out, aux_ids, fore_out, fore_ids, back_out = net(
    #                 images)
    #             ############ up ###########
    #             out_up = out_up.float()
    ...
    for e in range(begin_epoch, epoch):
        ...
        for i, dat in enumerate(train_loader):
            ...
            cam_up, out_up, pred_sort_up, pred_ids_up, cam_down, out_down, pred_sort_down, pred_ids_down, diff_out, diff_ids, aux_out, aux_ids, fore_out, fore_ids, back_out = net(images)
            ############ up ###########  删掉 cls_loss_up 的从 2022-08-27 开始。 加上 cls_loss_up 从 2022-08-28 23 开始
            cls_loss_up = cls_criterion(out_up, labels)
            loss = cls_loss_up
            cls_loss_up_cpu = cls_loss_up.item()
            ...
            if args.spa_loss == 1:
                activation_maps = torch_cam_combination(cam_down, None, pred_ids_down, args.function, args.mean_num)  #[bz, h*w]
                ...
                activation_maps_spa = torch_cam_combination(cam_down, None, pred_ids_down, args.function, args.mean_num)  # [bz, h*w]
                activation_maps_spa = activation_maps_spa.reshape(bz, h, w)
                activation_maps_spa = net.upsample(activation_maps_spa.unsqueeze(dim=1).to(cfg.device)).to(cfg.device).squeeze(
                        dim=1).to(
                        cfg.device)
        ...
        if args.dataset == "cub":
            ...
            if (e+1) % 5 == 0:
                train_avg_epoch_acc = epoch_acc / (train_data_length*4)
