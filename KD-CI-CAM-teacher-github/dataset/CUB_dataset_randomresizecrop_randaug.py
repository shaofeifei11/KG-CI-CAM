...
class CUB_Dataset_RandomResizeCrop_RandAug(Dataset):
    """Face Landmarks dataset."""

    def __init__(self, input_size, train=True, crop=False, danet=False, cam_img=False, min_scale=0.2, max_scale=1.0):
            ...
            if self.train:
                ...
                if danet:
                    self.func_transforms = transforms.Compose(
                        [transforms.RandomResizedCrop(cfg.crop_size, scale=(min_scale, max_scale)),
        ...
        with open(os.path.join(self.dir, "datalist", "test_bounding_box.txt"), 'r') as f:
            ...
            for i in range(len(lines)):
                ...
                self.bbox_list.append(
                    [[float(line[0]), float(line[1]), float(line[2]), float(line[3])]])  # [[xmin, ymin, xmax, ymax]]
