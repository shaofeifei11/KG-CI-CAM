...
def poly_lr_scheduler(optimizer, init_lr, iter, lr_decay_iter=1, max_iter=100, power=0.9, dataset="imagenet", backbone_rate=0.1, decay_rate=0.5, decay_epoch=2):
    ...
    for param_group in optimizer.param_groups:
        param_group['lr'] = lr
    if dataset == "imagenet":
        # optimizer.param_groups[-3]['lr'] = optimizer.param_groups[-3]['lr'] * backbone_rate   # 无 regressor
        ...
        # shang = max(iter // decay_epoch, 2)   max 效果非常差
        shang = iter // decay_epoch
        new_rate = 1
...
def poly_lr_scheduler_imagenet(optimizer, init_lr, iter, max_iter=100, power=0.9, dataset="imagenet", backbone_rate=0.1, decay_rate=0.5, decay_epoch=2, inception_reduce=0.1, max_num=100):
    print("poly_lr_scheduler_imagenet")
...
def poly_lr_scheduler_kd(optimizer, init_lr, iter, max_iter=100, power=0.9, dataset="imagenet", backbone_rate=0.1, decay_rate=0.5, decay_epoch=2, inception_reduce=0.1, max_num=100):
    print("poly_lr_scheduler_kd")
    lr = init_lr*(1 - iter/max_iter)**power
    if cfg.args.backbone == "inceptionV3" and iter >= 85:
        lr = lr * inception_reduce
    for param_group in optimizer.param_groups:
...
# def poly_lr_scheduler_1(optimizer, init_lr, iter, lr_decay_iter=1, max_iter=100, power=0.9, dataset="imagenet", backbone_rate=0.1, decay_rate=0.5, decay_epoch=2):
#     """Polynomial decay of learning rate
#         :param init_lr is base learning rate
#         :param iter is a current iteration
#         :param lr_decay_iter how frequently decay occurs, default is 1
...
#         optimizer.param_groups[-1]['lr'] = optimizer.param_groups[-1]['lr'] * backbone_rate
#         shang = iter // decay_epoch
#         lg = max(0, int(np.log2(shang + 0.1) + 1))
#         new_rate = 1
#         for i in range(lg):
#             new_rate = new_rate * decay_rate
#         for param_group in optimizer.param_groups:
...
# def poly_lr_scheduler_2(optimizer, init_lr, iter, lr_decay_iter=1, max_iter=100, power=0.9, dataset="imagenet", backbone_rate=0.1, decay_rate=0.5, decay_epoch=2, poly_max=2):
#     """Polynomial decay of learning rate
#         :param init_lr is base learning rate
#         :param iter is a current iteration
#         :param lr_decay_iter how frequently decay occurs, default is 1
...
#
#
# def poly_lr_scheduler_3(optimizer, init_lr, iter, lr_decay_iter=1, max_iter=100, power=0.9, dataset="imagenet", backbone_rate=0.1, decay_rate=0.5, decay_epoch=2):
#     """Polynomial decay of learning rate
#         :param init_lr is base learning rate
#         :param iter is a current iteration
#         :param lr_decay_iter how frequently decay occurs, default is 1
...
#             param_group['lr'] = param_group['lr'] * new_rate
#
# def poly_lr_scheduler_4(optimizer, init_lr, iter, lr_decay_iter=1, max_iter=100, power=0.9, dataset="imagenet", backbone_rate=0.1, decay_rate=0.5, decay_epoch=2, poly_max=2):
#     """Polynomial decay of learning rate
#         :param init_lr is base learning rate
#         :param iter is a current iteration
#         :param lr_decay_iter how frequently decay occurs, default is 1
...
def localization_acc_list(boxA_list, boxB_list, class_match):
    for boxA in boxA_list:
        if localization_acc(boxA, boxB_list, class_match) == 1.0:
            return 1.0
    return 0.0
...
def old_create_attention_low(labels, pred_ids, cam):
    bz, nc, h, w = cam.shape
    attention4 = torch.zeros(size=[bz, nc, cfg.attention_size*2, cfg.attention_size*2],  # todo
                             requires_grad=False).type(cam.dtype).to(cfg.device)
...
def old_create_attention_autocast(labels, pred_ids, cam):
...
def get_mask_cutmix(pred_ids, cam, upsample, seg_thr):
    bz, nc, h, w = cam.shape
    cam_bz_nc_index = get_pre_two_source_inds(cam.shape)
    ...
    activation_maps = activation_maps.reshape(bz, h, w)
    activation_maps = upsample(activation_maps.unsqueeze(dim=1).to(cfg.device)).to(cfg.device).squeeze(dim=1).to(
                cfg.device)

    activation_maps = activation_maps.reshape([bz, cfg.crop_size * cfg.crop_size])
    bounding_box_thr = torch.mul(torch.max(activation_maps, dim=1, keepdim=True)[0], seg_thr).to(cfg.device)
    foreground_mask = torch.where(torch.gt(activation_maps, bounding_box_thr).to(cfg.device), torch.ones(activation_maps.shape).to(cfg.device),
                                     torch.zeros(activation_maps.shape).to(cfg.device))
    background_mask = torch.where(torch.gt(activation_maps, bounding_box_thr).to(cfg.device), torch.zeros(activation_maps.shape).to(cfg.device),
                                     torch.ones(activation_maps.shape).to(cfg.device))
    return foreground_mask, background_mask


def get_mask(pred_ids, cam, upsample, seg_thr, combination=False, function="quadratic", mean_num=1):
    bz, nc, h, w = cam.shape
    if combination:
        activation_maps = torch_cam_combination(cam, None, pred_ids, function, mean_num)
        activation_maps = activation_maps.reshape(bz, h, w)
        activation_maps = upsample(activation_maps.unsqueeze(dim=1).to(cfg.device)).to(cfg.device).squeeze(dim=1).to(
            cfg.device)
        ...
        activation_maps = activation_maps.reshape([bz, cfg.crop_size * cfg.crop_size])
        bounding_box_thr = torch.mul(torch.max(activation_maps, dim=1, keepdim=True)[0], seg_thr).to(cfg.device)
        foreground_mask = torch.where(torch.gt(activation_maps, bounding_box_thr).to(cfg.device),
                                      torch.ones(activation_maps.shape).to(cfg.device),
                                      torch.zeros(activation_maps.shape).to(cfg.device))
        # background_mask = torch.where(torch.gt(activation_maps, bounding_box_thr).to(cfg.device),
        #                               torch.zeros(activation_maps.shape).to(cfg.device),
        #                               torch.ones(activation_maps.shape).to(cfg.device))

        background_mask = torch.sub(torch.ones(activation_maps.shape).to(cfg.device), foreground_mask)
        return foreground_mask, background_mask
...
def net_save_classifier(net, net_path):
    torch.save(net.state_dict(), net_path)
