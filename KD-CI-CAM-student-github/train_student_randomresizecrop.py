...
# from test_center_crop import inference  # todo 使用 center_crop_test 的代码
...
if __name__ == '__main__':
    ...
    optimizer = torch.optim.Adam([{'params': student_net.up_classifier.parameters()},
                                 {'params': student_net.mask2attention.parameters()},
                                 {'params': student_net.backbone.parameters()}], lr=lr)
    ...
    for e in range(begin_epoch, epoch):
        ...
        for i, dat in enumerate(train_loader):
            ...
            if cfg.attention:
               student_net.update_mask(old_create_attention(labels=labels, pred_ids=pred_ids_up, cam=cam_up))
        ...
        if args.dataset == "cub":
            if (e >= 49 and (e+1) % 5 == 0) or epoch - (e+1) < 5:
                net_save(student_net, os.path.join(net_dir, 'net_train_{}_{}_{}.pth'.format(args.backbone, args.dataset, e + 1)))
            if (e+1) % 5 == 0:
                ...
                c1, l1, c5, l5, gt_l = inference(student_net, args)
