...
class ImageNet_Dataset_KD_RandomReizeCrop(Dataset):
    """Face Landmarks dataset."""
    def __init__(self, danet=False, zero=True, cls_min_scale=0.2, cls_max_scale=1.0, loc_min_scale=0.1, loc_max_scale=1.0, distillation="random"):
        ...
        if danet:
            self.func_transforms_stu = transforms.Compose(
                [transforms.RandomResizedCrop(cfg.crop_size, scale=(cls_min_scale, cls_max_scale)),
            ...
            self.func_transforms_loc = transforms.Compose(
                [transforms.RandomResizedCrop(cfg.crop_size, scale=(loc_min_scale, loc_max_scale)),
                 transforms.RandomHorizontalFlip(),
                 transforms.ColorJitter(0.4, 0.4, 0.4),
                 transforms.ToTensor(),
                 Lighting(0.1, _imagenet_pca['eigval'], _imagenet_pca['eigvec']),
    ...
    def read_labeled_image_list(self):
        ...
        with open(self.list_path, 'r') as f:
            for line in f:
                ls = line.strip("\n").split(' ')
                self.image_list.append(os.path.join(basedir, ls[-2]))
