...
from utils import create_rois, get_pre_two_source_inds, compute_gt_rois, get_mask, create_rois_border
...
class CUB_VGG16(nn.Module):
    def __init__(self, args):
        ...
        self.backbone = Non_Local_VGG16(cfg.pretrain)
    ...
    def aux_classification(self, x):
        foreground_mask, background_mask = get_mask(self.pred_ids_down, self.down_cam, self.upsample,
                                                    ...
                                                    combination=cfg.combination, function=cfg.function,
                                                    mean_num=cfg.mean_num)
        ...
        # aux = torch.add(self.softmax(diff_out), self.softmax(self.down_out))
        _, aux_ids = torch.sort(aux_out, dim=-1, descending=True)
        _, diff_ids = torch.sort(diff_out, dim=-1, descending=True)
        return diff_out, diff_ids, aux_out, aux_ids
    ...
    def update_mask(self, tmp_mask):
        ...
        self.mask = torch.add(self.mask, torch.mul(tmp_mask, cfg.update_rate).to(cfg.device)).to(cfg.device)
    ...
    def compute_rois_border(self, seg_thr, topk, combination, border_images):
        """
        :param pred:  [batch_size, classes]
        :param cam:  [batch_size, channel, height, width]
        :return:
        """
        return create_rois_border(self.pred_sort_down, self.pred_ids_down, self.down_cam, self.upsample, seg_thr=seg_thr, topk=topk,
                           combination=combination, function=cfg.function, mean_num=cfg.mean_num, border_images=border_images)  # [batch, topk, 4]
