...
def create_rois_border(pred_sort, pred_ids, cam, upsample, seg_thr, topk=1, combination=False, function="quadratic", mean_num=1, border_images=None):
    """
    ...
    if combination:
        ...
        border_images = border_images.reshape([bz, cfg.crop_size * cfg.crop_size])
        activation_maps = torch.mul(activation_maps, border_images)
...
def net_load(net, net_path):
    print(net_path)
    # try:
    #     weight = torch.load(net_path, map_location=torch.device("cuda"))
    # except Exception:
    #     with open(net_path, 'rb') as f:
    #         buffer = io.BytesIO(f.read())
    #     weight = torch.load(buffer)
