...
def inference(net, args):
    ...
    if args.dataset == "cub":
        test_data_crop = CUB_Dataset(input_size=args.cls_teacher_input_size, train=False, crop=True)
        test_data = CUB_Dataset(input_size=args.cls_teacher_input_size, train=False)
    else:
        test_data_crop = ImageNet_Dataset(input_size=args.cls_teacher_input_size, train=False, crop=True)
        test_data = ImageNet_Dataset(input_size=args.cls_teacher_input_size, train=False)
...
if __name__ == '__main__':
    ...
    if args.dataset == "cub":
        if args.backbone == "vgg16":
            ...
            net = CUB_InceptionV3_NonLocal(args)
    else:
        if args.backbone == "vgg16":
            net = ImageNet_VGG16(args)
